...
// 修改密码
router.post('/manage/user/updatepwd', (req, res) => {
  const { userId, oldPassword, password } = req.body
  console.log(userId,password,oldPassword, md5(oldPassword));
  
  UserModel.findOneAndUpdate({ _id: userId, password: md5(oldPassword) }, { password: md5(password) })
    .then(oldUser => {
      // const data = Object.assign(oldUser, user)
      console.log(oldUser);
      if (oldUser) {
        res.send({ status: 0, oldUser })
      } else {
        res.send({ status: 2, msg: '原密码错误' })
      }
    ...
    .catch(error => {
      ...
      res.send({ status: 1, msg: '更新密码异常, 请重新尝试' })
    })
})
...
// 删除商品
router.post('/mange/product/delete', (req, res) => {
